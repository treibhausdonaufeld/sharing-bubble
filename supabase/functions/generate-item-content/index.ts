...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Generating AI content for job: ${jobId} with image: ${primaryImageUrl} in language: ${userLanguage}`);
    ...
    const aiContent = await generateAIContent(base64Image, mimeType, userLanguage, geminiApiKey);
...
async function generateAIContent(base64Image: string, mimeType: string, userLanguage: string, apiKey: string) {
...
Format your response as JSON:
{
  ...
  try {
    ...
    const title = lines[0]?.replace(/^title:?\s*/i, '').substring(0, 60) || 'Quality Item';
    const description = lines.slice(1).join(' ') || 'A quality item in good condition.';
